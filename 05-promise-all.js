...
var getFirstComments = async () => {
	...
	response = await testGet(url3 + posts[0].id);
	// Simulating error
	if(simulateError) throw "A simulated error has occurred...";
	var comments = JSON.parse(response);
	return comments[0];
};
...
var getAll2 = (async () => { // IIFE
	var arr = await Promise.all([getComments(),getFirstComments()]).catch(catchError);7
